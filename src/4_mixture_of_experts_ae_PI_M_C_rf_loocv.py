...
for loop, (X_train_PI, X_validation_PI, X_test_PI, 
    ...
    autoencoder_C, encoder_C = build_autoencoder(
        input_dim=X_train_C.shape[1],
        latent_dim=best_params_C["latent_dim"],
        dropout=best_params_C["dropout"],
        l2_reg=best_params_C["l2_reg"])

    print("🟢 Compile Autoencoder C")
    autoencoder_C.compile(optimizer=Adam(learning_rate=best_params_C["lr"]), loss="mse")

    print("🟢 Train Autoencoder C")
    autoencoder_C.fit(
        X_train_C, X_train_C,
        validation_data=(X_validation_C, X_validation_C),
        epochs=50,
        batch_size=64,
        callbacks=[EarlyStopping(
            monitor="val_loss",       # which metric to monitor
    ...
    print("🟢 Optimize RandomForest hyperparameters PI")
    study_rf_PI = optuna.create_study(direction="maximize")
    study_rf_PI.optimize(
        lambda trial: objective_rf(trial, Z_train_PI, y_train, Z_validation_PI, y_validation),
        n_trials=args.optimize_trials
    )

    best_params_rf_PI = study_rf_PI.best_trial.params
    print(best_params_rf_PI)

    print("🟢 Build classifier PI with best RandomForest parameters")
    clf_PI = RandomForestClassifier(
        n_estimators=best_params_rf_PI["n_estimators"],
        max_depth=best_params_rf_PI["max_depth"],
        min_samples_leaf=best_params_rf_PI["min_samples_leaf"],
        min_samples_split=best_params_rf_PI["min_samples_split"],
        max_features=best_params_rf_PI["max_features"],
        class_weight="balanced",
        n_jobs=-1,
        random_state=42
    )
    ...
    print("🟢 Optimize RandomForest hyperparameters M")
    study_rf_M = optuna.create_study(direction="maximize")
    study_rf_M.optimize(
        lambda trial: objective_rf(trial, Z_train_M, y_train, Z_validation_M, y_validation),
        n_trials=args.optimize_trials
    )

    best_params_rf_M = study_rf_M.best_trial.params
    print(best_params_rf_M)

    print("🟢 Build classifier M with best RandomForest parameters")
    clf_M = RandomForestClassifier(
        n_estimators=best_params_rf_M["n_estimators"],
        max_depth=best_params_rf_M["max_depth"],
        min_samples_leaf=best_params_rf_M["min_samples_leaf"],
        min_samples_split=best_params_rf_M["min_samples_split"],
        max_features=best_params_rf_M["max_features"],
        class_weight="balanced",
        n_jobs=-1,
        random_state=42
    )
    ...
    Z_validation_C = encoder_C.predict(X_validation_C)

    print("🟢 Optimize RandomForest hyperparameters C")
    study_rf_C = optuna.create_study(direction="maximize")
    study_rf_C.optimize(
        lambda trial: objective_rf(trial, Z_train_C, y_train, Z_validation_C, y_validation),
        n_trials=args.optimize_trials
    )

    best_params_rf_C = study_rf_C.best_trial.params
    print(best_params_rf_C)

    print("🟢 Build classifier C with best RandomForest parameters")
    clf_C = RandomForestClassifier(
        n_estimators=best_params_rf_C["n_estimators"],
        max_depth=best_params_rf_C["max_depth"],
        min_samples_leaf=best_params_rf_C["min_samples_leaf"],
        min_samples_split=best_params_rf_C["min_samples_split"],
        max_features=best_params_rf_C["max_features"],
        class_weight="balanced",
        n_jobs=-1,
        random_state=42
    )
    ...
    f1_score_test_C = f1_score(y_test, y_test_pred_M, average='macro')
